...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    /**
     * Complete a job and notify the user and translator
     *
     * @param array $postData Array containing the job ID and user ID
     */
    public function jobEnd(array $postData)
    {
        // Get the job and calculate the session time
        $job = Job::with('translatorJobRel')->find($postData['job_id']);
        $dueDate = new DateTime($job->due);
        $completedDate = new DateTime();
        $interval = $dueDate->diff($completedDate)->format('%h:%i:%s');

        // Update the job status and session time
        ...
        // Fire event to notify the user and translator that the session has ended
        $translator = $job->translatorJobRel->where('completed_at', null)->where('cancel_at', null)->first();
        $userId = ($postData['user_id'] == $job->user_id) ? $translator->user_id : $job->user_id;
        ...
        // Update the translator job relation with the completion date and user ID
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestdata['translator_email']) && $requestdata['translator_email'] != '') {
                ...
                if ($user) {
                    $allJobIDs = TranslatorJobRel::where('user_id', $user->id)->pluck('job_id')->toArray();
                    $allJobs->whereIn('jobs.id', $allJobIDs);
                }
            }
            if (isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "created") {
    ...
    public function ignore($modelClass, $id, $field)
    {
    ...
    {
        return $this->ignore(Job::class, $id, 'ignore');
    }

    public function ignoreExpired($id)
    {
        return $this->ignore(Job::class, $id, 'ignore_expired');
    }

    public function ignoreThrottle($id)
    {
        return $this->ignore(Throttles::class, $id, 'ignore');
    }
